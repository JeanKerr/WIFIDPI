...
void* safe_malloc(size_t size)
{
    ...
    retval = malloc(size);
    if (!retval) {
        debug(LOG_CRIT, "Failed to malloc %lu bytes of memory: %s.  Bailing out", size, strerror(errno));
        rhy_exit(-2);
    }
...
void* safe_realloc(void* ptr, size_t newsize)
{
    ...
    if (NULL == retval) {
        debug(LOG_CRIT, "Failed to realloc buffer to %lu bytes of memory: %s. Bailing out", newsize, strerror(errno));
        rhy_exit(-3);
    }
    return retval;
}
