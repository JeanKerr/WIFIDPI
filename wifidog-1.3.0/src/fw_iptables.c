 ...
 */
static void iptables_insert_gateway_id(char **input)
{
    ...
    while ((token = strstr(*input, "$ID$")) != NULL)
        memcpy(token, "%1$s", 4);   /* This string may look odd but it's standard POSIX and ISO C */
...
static int iptables_do_command(const char *format, ...)
{
    ...
    safe_asprintf(&cmd, "iptables %s -w", fmt_cmd);
    free(fmt_cmd);

    iptables_insert_gateway_id(&cmd);

    debug(LOG_INFO, "Executing command: %s", cmd);
 ...
 */
static char* iptables_compile(const char *table, const char *chain, const t_firewall_rule * rule)
{
    ...
    snprintf(command, sizeof(command)-1, "-t %s -A %s ", table, chain);
    if (rule->mask != NULL) {
        if (rule->mask_is_ipset) {
            snprintf((command + strlen(command)), (sizeof(command)-1 -
                                                   strlen(command)), "-m set --match-set %s dst ", rule->mask);
        } else {
            snprintf((command + strlen(command)), (sizeof(command)-1 - strlen(command)), "-d %s ", rule->mask);
        }
    }
    if (rule->protocol != NULL) {
        snprintf((command + strlen(command)), (sizeof(command)-1 - strlen(command)), "-p %s ", rule->protocol);
    }
    if (rule->port != NULL) {
        snprintf((command + strlen(command)), (sizeof(command)-1 - strlen(command)), "--dport %s ", rule->port);
    }
    snprintf((command + strlen(command)), (sizeof(command)-1 - strlen(command)), "-j %s", mode);
 ...
 */
static void iptables_load_ruleset(const char *table, const char *ruleset, const char *chain)
{
    ...
    for (rule = get_ruleset(ruleset); rule != NULL; rule = rule->next) {
        ...
        if (cmd != NULL) {
            debug(LOG_INFO, "Loading rule \"%s\" into table %s, chain %s", cmd, table, chain);
...
void iptables_fw_clear_authservers(void)
{
    ...
    //iptables_do_command("-t filter -F INPUT");
}
...
void iptables_fw_set_authservers(void)
{
    ...
    for (auth_server = get_auth_server(); auth_server != NULL; auth_server = auth_server->next) 
    {
        if (auth_server->last_ip && strcmp(auth_server->last_ip, "0.0.0.0") != 0) 
        {
            ...
            #if 0
            iptables_do_command("-t filter -A INPUT -s %s -p tcp --dport %d -j ACCEPT", 
                                    auth_server->last_ip, config->external_web_port);
            #endif
        }
    }
 ...
 * This is used when we do a clean shutdown and when it starts to make
...
int iptables_fw_destroy_mention(const char *table, const char *chain, const char *mention)
{
    ...
    safe_asprintf(&command, "iptables -t %s -L %s -n --line-numbers", table, chain);
...
int iptables_fw_rule_replace(const char *ip, const char *mac, int tag)
{
    char *command = NULL;
    char rulenum[10];
    int replaced = 0;
    FILE *output;
    char *script, rc;
    char ipFind[MAX_IP_ADDR_LEN]={0};
    char macFind[MAX_MAC_ADDR_LEN]={0};
    
    /* Look for outgoing traffic */
    safe_asprintf(&script, "iptables -t mangle -L " CHAIN_OUTGOING " -n --line-numbers");
    iptables_insert_gateway_id(&script);
    debug(LOG_DEBUG, "Executing command: %s", script);
    ...
    {
        rc = fscanf(output, "%15[0-9] %*s %*s %*s %15[0-9.] %*s MAC %15[0-9A-Z:] MARK set %*s", rulenum, ipFind, macFind);
        if (3 == rc && EOF != rc) 
        {
            /* Sanity */
            if (0==strcmp(ip, ipFind) && 0==strcmp(mac, macFind))
            {
                /* Replace the rule: */
                debug(LOG_DEBUG, "Replace rule %s from mangle." CHAIN_OUTGOING "%s MAC %s MARK set %d", rulenum, ip, mac, tag);
                safe_asprintf(&command, "-t mangle -R " CHAIN_OUTGOING " %s -s %s -m mac --mac-source %s -j MARK --set-mark %d", 
                    rulenum, ip, mac, tag);
    ...
    /* Look for outgoing traffic */
    safe_asprintf(&script, "iptables -t mangle -L " CHAIN_INCOMING " -n --line-numbers");
    iptables_insert_gateway_id(&script);
    debug(LOG_DEBUG, "Executing command: %s", script);
    output = popen(script, "r");
    free(script);
    ...
    while (output && !(feof(output))) 
    {
        rc = fscanf(output, "%15[0-9] %*s %*s %*s %15[0-9.]", rulenum, ipFind);
        if (3 == rc && EOF != rc) 
        {
            /* Sanity */
            if (0==strcmp(ip, ipFind))
            {
                /* Replace the rule: */
                debug(LOG_DEBUG, "Replace rule %s from mangle." CHAIN_INCOMING "%s", rulenum, ip);
                safe_asprintf(&command, "-t mangle -R " CHAIN_INCOMING " %s -s %s -m mac --mac-source %s -j MARK --set-mark %d", 
                    rulenum, ip, mac, tag);
                iptables_do_command(command);
                free(command);
                replaced++;
...
bool iptables_fw_access(fw_access_t type, const char *ip, const char *mac, int tag)
{
    ...
    switch (type)
    {
        case FW_ACCESS_ALLOW:
            ...
            break;
        case FW_ACCESS_DENY:
            /* XXX Add looping to really clear? no, this will cause multi-threads wait lock */
            #if 1
            ...
            #endif
            break;
        case FW_ACCESS_REPLACE:
            rc = iptables_fw_rule_replace(ip, mac, tag);
            if(rc!=2)
            {
                iptables_fw_access(FW_ACCESS_DENY, ip, mac, tag);
                rc = iptables_fw_access(FW_ACCESS_ALLOW, ip, mac, tag);
            }
            else
            {
                rc = 0;
...
int iptables_fw_counters_update(void)
{
    ...
    unsigned long long int counterPkt;
    unsigned long long int counterByt;
    t_client *p1;
    ...
    iptables_insert_gateway_id(&script);
    debug(LOG_TRACE, "Executing command: %s", script);
    output = popen(script, "r");
    free(script);
    if (!output)
    {
    ...
    while (output && !(feof(output))) 
    {
        rc = fscanf(output, "%llu %llu %*s %*s %*s %*s %*s %15[0-9.] %*s %*s %*s %*s %*s %*s", &counterPkt, &counterByt, ip);
        ...
        if (3 == rc && EOF != rc) 
        {
            ...
            {
                debug(LOG_WARNING, "Supposed to read an IP address but instead got [%s] - ignoring it", ip);
                continue;
            }
            debug(LOG_TRACE, "Read outgoing counters for %s: Pkts=%llu Bytes=%llu", ip, counterPkt, counterByt);
            LOCK_CLIENT_LIST();
            if ((p1 = client_list_find_by_ip(ip)))
            {
                if ((p1->counters.outGoingByt - p1->counters.outGoingBytHistory) < counterByt) 
                {
                    p1->counters.outGoingPktDelta = p1->counters.outGoingPktHistory + counterPkt - p1->counters.outGoingPkt;
                    p1->counters.outGoingPkt = p1->counters.outGoingPktHistory + counterPkt;
                    p1->counters.outGoingBytDelta = p1->counters.outGoingBytHistory + counterByt - p1->counters.outGoingByt;
                    p1->counters.outGoingByt = p1->counters.outGoingBytHistory + counterByt;
                    p1->counters.last_updated = time(NULL);
                    debug(LOG_TRACE, "%s outgoing counters %llu pkts %llu bytes, updated to %llu pkts %llu bytes. last_updated %lu", 
                          ip, counterPkt, counterByt, p1->counters.outGoingPkt, p1->counters.outGoingByt, p1->counters.last_updated);
                }
            }
            else 
            {
                debug(LOG_ERR, "iptables_fw_counters_update() could not find %s, this should not happen unless crashed", ip);
                debug(LOG_INFO, "Preventively deleting firewall rules for %s in table %s", ip, CHAIN_OUTGOING);
                iptables_fw_destroy_mention("mangle", CHAIN_OUTGOING, ip);
                debug(LOG_INFO, "Preventively deleting firewall rules for %s in table %s", ip, CHAIN_INCOMING);
                iptables_fw_destroy_mention("mangle", CHAIN_INCOMING, ip);
            }
            UNLOCK_CLIENT_LIST();
        }
    ...
    while (output && !(feof(output))) 
    {
        rc = fscanf(output, "%llu %llu %*s %*s %*s %*s %*s %*s %15[0-9.]", &counterPkt, &counterByt, ip);
        if (3 == rc && EOF != rc)
        {
            /* Sanity */
            if (!inet_aton(ip, &tempaddr))
            ...
            debug(LOG_TRACE, "Read incoming counters for %s: Pkts=%llu Bytes=%llu", ip, counterPkt, counterByt);
            LOCK_CLIENT_LIST();
            if ((p1 = client_list_find_by_ip(ip)))
            {
                if ((p1->counters.inComingByt - p1->counters.inComingBytHistory) < counterByt)
                {
                    p1->counters.inComingPktDelta = p1->counters.inComingPktHistory + counterPkt - p1->counters.inComingPkt;
                    p1->counters.inComingPkt = p1->counters.inComingPktHistory + counterPkt;
                    p1->counters.inComingBytDelta = p1->counters.inComingBytHistory + counterByt - p1->counters.inComingByt;
                    p1->counters.inComingByt = p1->counters.inComingBytHistory + counterByt;
                    debug(LOG_TRACE, "%s incoming counters %llu pkts %llu bytes, updated to %llu pkts %llu bytes.", 
                          ip, counterPkt, counterByt, p1->counters.inComingPkt, p1->counters.inComingByt);
                }
            }
            else
            {
