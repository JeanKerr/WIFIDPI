...
/**
 * A flag to denote whether we were restarted via a parent process, or started normally
 ...
 * @brief Print usage
 *
 * Prints usage, called when process is run with -h or with an unknown option
 */
static void usage(char* process)
{
    ...
    fprintf(stdout, "  -x pid        Used internally by %s when re-starting itself *DO NOT ISSUE THIS SWITCH MANUAlLY*\n", config_get_config()->company);
...
void parse_commandline(int argc, char **argv)
{
    ...
    while (-1 != (c = getopt(argc, argv, "c:hfd:sw:vx:i:a:V"))) {
        ...
        case 'V':
            fprintf(stdout, "Based on Wifidog " VERSION "\n");
            exit(1);
            break;
        ...
        case 'i':
            if (optarg) {
                CONFIG_SET(internal_sock, optarg);
            }
            break;

        case 'a':
            if (optarg) {
                CONFIG_SET(arp_table_path, optarg);
            } else {
        ...
        case 'p':
            if (optarg) {
                CONFIG_SET(pidfile, optarg);
            } else {
                fprintf(stdout, "The expected PID file path to the %s was not supplied!\n", argv[0]);
                exit(1);
            }
            break;
        default:
