...
char* http_get2(const int sockfd, const char *req, const char ending)
{
    ssize_t numbytes;
    int done, nfds;
    fd_set readfds;
    ...
    if (numbytes <= 0) {
        ...
        debug(LOG_ERR, "send failed: only %lu bytes out of %lu bytes sent!", numbytes, reqlen);
        goto error;
    }
    ...
    timeout.tv_sec = MAX_SELECT_SECONDS;    /* original:30 seconds, now:10 seconds */
    timeout.tv_usec = 0;
        ...
        if (nfds > 0) 
        {
            ...
            *  was only one fd. */
            ...
            else
            {
                ...
                if(!strchr(readbuf, ending))
                {
                    debug(LOG_DEBUG, "Read %lu bytes", numbytes);
                    memset(readbuf, 0, numbytes);
                }
                else
                {
                    done = 1;
                    debug(LOG_DEBUG, "Read %lu bytes with ending char %c", numbytes, ending);
                }
            }
        }
        else if (nfds == 0) 
    ...
    debug(LOG_DEBUG, "HTTP Response from Server:\nMessage Begin:\n-------------->\n%s\n<--------------\nMessage End\n", retval);
    return retval;
...
char* http_get(const int sockfd, const char *req)
{
    ssize_t numbytes;
    int done, nfds;
    fd_set readfds;
    ...
    do {
        FD_ZERO(&readfds);
        FD_SET(sockfd, &readfds);
        timeout.tv_sec = MAX_SELECT_SECONDS;    /* original:30 seconds, now:3 seconds */
        timeout.tv_usec = 0;
        nfds = sockfd + 1;
